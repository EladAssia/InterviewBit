...
# product of the following:<ul>

# LeftSpecialValue: For an index i, it is defined as the index j such that A[j]>A[i] (i>j). If multiple A[j]’s are present 
# in multiple positions, the LeftSpecialValue is the maximum value of j. 

# RightSpecialValue: For an index i, it is defined as the index j such that A[j]>A[i] (j>i). If multiple A[j]s are present 
# in multiple positions, the RightSpecialValue is the minimum value of j.
